...
```{r}
generate.data <- function(n.participants, n.measurements, fixed.coefficient.1, fixed.coefficient.2, fixed.coefficient.3, fixed.coefficient.4, sd.random.intercept, sd.error) {
  ...
  fixed.effect <- c(x.matrix %*% c(fixed.coefficient.1, fixed.coefficient.2, fixed.coefficient.3, fixed.coefficient.4))
...
estimate.model.and.get.coefficients.nlme <- function(data) {
  ptm <- proc.time()
  model <- lme(
    fixed = y ~ 0 + bbase(x, xl = 0, xr = 1, nseg = 1, bdeg = 3),
    data = data,
...
estimate.model.and.get.coefficients.lme4 <- function(data) {
  ptm <- proc.time()
  model <- lmer(
    formula = y ~ 0 + bbase(x, xl = 0, xr = 1, nseg = 1, bdeg = 3) + (1 | participant.id),
    data = data,
    REML = FALSE
  )
  time <- proc.time() - ptm
...
estimate.model.and.get.coefficients.gamlss1 <- function(data) {
  ...
  ptm <- proc.time()
  model <- gamlss(
    formula = y ~ 0 + re(fixed = ~ 0 + bbase(x, xl = 0, xr = 1, nseg = 1, bdeg = 3), random = ~ 1 | participant.id),
    sigma.formula = ~ 1,
    family = NO(
      mu.link = "identity",
      sigma.link = "log"
  ...
  result <- c(
    ...
    fixed.coefficient.4 = fixef(getSmo(model))[[4]],
...
estimate.all.models.and.get.coefficients <- function(data) {
  result <- data.frame(
    method = c("nlme", "lme4", "gamlss1", "gamlss2", "mgcv1", "mgcv2", "optim", "gls"),
    rbind(
...
parameters <- tibble(
  parameter.set = seq(6),
  n.participants = c(200, 800, 200, 200, 200, 200),
  ...
  fixed.coefficient.4 = c(1, 1, 1, -0.5, 1, 1),
...
ggplot(
  ...
  data = estimated.parameters %>%
    ...
    reframe(
      expand_grid(
        parameter.1 = c("fixed.coefficient.1", "fixed.coefficient.2", "fixed.coefficient.3", "fixed.coefficient.4", "sd.random.intercept", "sd.error"),
        parameter.2 = c("fixed.coefficient.1", "fixed.coefficient.2", "fixed.coefficient.3", "fixed.coefficient.4", "sd.random.intercept", "sd.error")
      ),
      correlation = c(cor(data.frame(fixed.coefficient.1, fixed.coefficient.2, fixed.coefficient.3, fixed.coefficient.4, sd.random.intercept, sd.error)))
    )
) +
  geom_tile(alpha = 0.5) +
  geom_text() +
