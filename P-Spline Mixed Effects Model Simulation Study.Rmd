---
title: "P-Spline Mixed Effects Linear Regression Simulation Study"
...
parameters <- tibble(
  parameter.set = seq(8),
  n.participants = c(200, 200, 800, 200, 200, 200, 200, 200),
  n.measurements = c(4, 4, 4, 16, 4, 4, 4, 4),
...
estimated.parameters <- parameters %>%
  ...
  reframe(
    generate.data(
      ...
      estimate.model.and.get.coefficients.gamlss2() %>%
      as_tibble_row(),
    .by = c(parameter.set, run)
  )
estimated.parameters
...
ggplot(
  ...
  data = estimated.parameters %>%
    ...
    reframe(
      expand_grid(
        parameter.1 = c("fixed.coefficient.1", "fixed.coefficient.2", "fixed.coefficient.3", "fixed.coefficient.4", "fixed.coefficient.5", "sd.random.intercept", "sd.error"),
        parameter.2 = c("fixed.coefficient.1", "fixed.coefficient.2", "fixed.coefficient.3", "fixed.coefficient.4", "fixed.coefficient.5", "sd.random.intercept", "sd.error")
      ),
      correlation = c(cor(data.frame(fixed.coefficient.1, fixed.coefficient.2, fixed.coefficient.3, fixed.coefficient.4, fixed.coefficient.5, sd.random.intercept, sd.error)))
    )
) +
  geom_tile(alpha = 0.5) +
  geom_text() +
